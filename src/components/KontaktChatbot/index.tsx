...
const Kontakt = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Students number validation (optional, must be a valid number)
    if (formData.students && !/^\d+$/.test(formData.students)) {
      toast.error("Bitte geben Sie eine gültige Zahl für die Anzahl der Schülerinnen und Schüler ein.");
      return;
    }
...
<div className="w-full px-4 md:w-1/2 xl:px-5">
                ...
                <div className="w-full px-4 md:w-1/2 xl:px-5">
                  <div className="mb-9.5">
                    <label htmlFor="students" className="mb-2.5 block font-medium text-white">
                      Wie viele SuS hat Ihre Schule?
                    </label>
                    <input
                      id="students"
                      type="number"
                      ...
                      onChange={handleChange}
                      placeholder="Anzahl der Schülerinnen und Schüler"
                      min="1"
                      className="w-full rounded-lg border border-white/[0.12] bg-white/[0.05] px-6 py-3 text-white outline-none focus:border-purple"
                    />
                  </div>
                ...
                <div className="w-full px-4 md:w-1/2 xl:px-5">
                  <div className="mb-9.5">
                    <label htmlFor="website" className="mb-2.5 block font-medium text-white">
                      Geben Sie hier die Website Ihrer Schule ein
                    </label>
                    <input
                      id="website"
                      type="text"
                      ...
                      onChange={handleChange}
                      placeholder="https://www.ihre-schule.de"
                      className="w-full rounded-lg border border-white/[0.12] bg-white/[0.05] px-6 py-3 text-white outline-none focus:border-purple"
                    />
                  </div>
                </div>
